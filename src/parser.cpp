...
Object *loadFromXML(const std::string &filename)
{
    ...
    if (!result) /* There was a parser / file IO error */
        throw SireException("Error while parsing \"%s\": %s (at %s)", filename, result.description(), offset(result.offset));
    ...
    auto check_attributes = [&](const pugi::xml_node &node, std::set<std::string> attrs) {
        for (auto attr : node.attributes()) {
            ...
            if (it == attrs.end())
                throw SireException("Error while parsing \"%s\": unexpected attribute \"%s\" in \"%s\" at %s",
                                    filename, attr.name(), node.name(), offset(node.offset_debug()));
            attrs.erase(it);
        }
        if (!attrs.empty())
            throw SireException("Error while parsing \"%s\": missing attribute \"%s\" in \"%s\" at %s",
    ...
    std::function<Object *(pugi::xml_node &, PropertyList &, int)> parseTag = [&](
        ...
        if (it == tags.end())
            throw SireException("Error while parsing \"%s\": unexpected tag \"%s\" at %s",
        ...
        if (!hasParent && !currentIsObject)
            throw SireException("Error while parsing \"%s\": root element \"%s\" must be a Nori object (at %s)",
                                filename, node.name(), offset(node.offset_debug()));

        if (parentIsTransform != currentIsTransformOp)
            throw SireException("Error while parsing \"%s\": transform nodes "
                                "can only contain transform operations (at %s)",
                                filename,  offset(node.offset_debug()));

        if (hasParent && !parentIsObject && !(parentIsTransform && currentIsTransformOp))
            throw SireException("Error while parsing \"%s\": node \"%s\" requires a Nori object as parent (at %s)",
                                filename, node.name(), offset(node.offset_debug()));
        ...
        try {
            if (currentIsObject) {
                ...
                case EMatrix: {
                    ...
                    if (tokens.size() != 16)
                        throw SireException("Expected 16 values");
                ...
                default: throw SireException("Unhandled element \"%s\"", node.name());
                };
            }
        } catch (const SireException &e) {
            throw SireException("Error while parsing \"%s\": %s (at %s)", filename,
