...
{
public:
    static void draw(nanogui::GLShader* prg)
    {
        Point3f points[4] = { Point3f(0.5, 0.5, 0.0), Point3f(-0.5, 0.5, 0.0), Point3f(-0.5, -0.5, 0.0), Point3f(0.5, -0.5, 0.0) };
        unsigned int vertexBufferId, indexBufferId;
        glGenBuffers(1,&vertexBufferId);
        glBindBuffer(GL_ARRAY_BUFFER, vertexBufferId);
        glBufferData(GL_ARRAY_BUFFER, sizeof(Point3f)*4, points[0].data(), GL_STATIC_DRAW);

        Eigen::Vector3i indices[2] = { Eigen::Vector3i(0,1,3), Eigen::Vector3i(3,1,2)};
        glGenBuffers(1,&indexBufferId);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferId);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Eigen::Vector3i)*2, indices[0].data(), GL_STATIC_DRAW);
